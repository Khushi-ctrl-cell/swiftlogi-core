...
import {
  ...
  SlidersHorizontal, Truck, WalletCards, Wrench, X, type LucideIcon,
} from "lucide-react";
...
import { adminNavigation, getModuleDefinition, moduleDefinitions, partnerNavigation } from "./mock-data";
import type { ModuleDefinition, PortalRole, TableRecord, Tone } from "./types";
...
function Dashboard({ role, onOpenOrders }: { role: PortalRole; onOpenOrders: () => void }) {
  ...
  const recent = getModuleDefinition("All Orders");
...
function CompactTable({ definition, limit, onView }: { definition: ModuleDefinition; limit?: number; onView: (record?: TableRecord) => void }) {
...
export function LogisticsApp() {
  ...
  return <div className={`app-shell ${collapsed ? "shell-collapsed" : ""}`}><Sidebar role={role} active={active} collapsed={collapsed} mobileOpen={mobileOpen} onSelect={setActive} onCollapse={() => setCollapsed(!collapsed)} onClose={() => setMobileOpen(false)} /><div className="app-main"><Topbar role={role} onRole={switchRole} onMenu={() => setMobileOpen(true)} onLogout={() => setAuthenticated(false)} /><main className="workspace">{isDashboard ? <Dashboard role={role} onOpenOrders={() => setActive("All Orders")} /> : <ModulePage moduleName={active} onOpen={(kind, record) => setModal(record ? { kind, record } : { kind })} />}</main></div>{mobileOpen && <button className="sidebar-scrim" onClick={() => setMobileOpen(false)} aria-label="Close navigation" />}
